...
import { DompileConfig } from '../types';
...
export class ConfigLoader {
    ...
    async loadConfig(): Promise<DompileConfig> {
    ...
    private getDefaultConfig(): DompileConfig {
        return {
            source: 'src',
            output: 'dist',
            includes: 'includes',
    ...
    private loadFromVSCodeSettings(): Partial<DompileConfig> {
        const config = vscode.workspace.getConfiguration('dompile');
    ...
    private async loadFromConfigFile(): Promise<Partial<DompileConfig>> {
        const configFiles = [
            'dompile.config.js',
            'dompile.config.json',
            'dompile.config.mjs',
        ...
        for (const configFile of configFiles) {
            ...
            if (fs.existsSync(configPath)) {
                try {
                    if (configFile.endsWith('.json') || configFile === '.dompilerc') {
    ...
    private async loadFromPackageJson(): Promise<Partial<DompileConfig>> {
        ...
        try {
            ...
            // Look for dompile config in package.json
            if (packageJson.dompile) {
                return packageJson.dompile;
            }

            // Infer config from scripts
            const config: Partial<DompileConfig> = {};
    ...
    async watchConfigChanges(callback: (config: DompileConfig) => void): Promise<vscode.Disposable> {
        const watchers: vscode.Disposable[] = [];

        // Watch VS Code settings
        const settingsWatcher = vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('dompile')) {
        ...
        const configPattern = new vscode.RelativePattern(
            this.workspaceRoot,
            '{dompile.config.*,.dompilerc*,package.json}'
        );
    ...
    validateConfig(config: DompileConfig): string[] {
        const errors: string[] = [];
